...
In this exercise, learners will be using Plan designer to build a complete targeted business solution that can include apps, workflows, agents and more. Copilot will guide you through the entire process of creating the solution.
...
Contoso Consulting is a professional services organization specializing in IT and AI consulting services. Since they have switched to a hybrid-work model, many of Contoso employees are working from home. This causes challenges as their employees need to checkout equipment often.
...
1.  To change the requirements, select the **Edit** button.
2.  In the **Copilot** field under the **User requirements** heading, enter: “*Staff should be able to report if there is an issue with the equipment while they have it checked out*.”
3.  Select the **Go** button.

A new issue reporting requirement will be added to your plan.

1.  Once you are happy with the requirements as they are listed, select the **Keep** button.
2.  If you are happy with the requirements generated by the **Requirements agent**, select **Looks good**.
...
1.  To view the details of the model, select **Show Details**.
...
1.  In the **Copilot** pane on the right side of the screen, enter the following: “*Add a text column to the Equipment table called Item number.*”
2.  After the column is added to the **Equipment** table, use **Copilot** to enter the following prompts individually:
...
1.  Using the navigation on the left, select the **Overview** icon. *(Located below the Back Arrow.)*
2.  In the **Data Agent**, select **Looks good** to advance to the next step.
...
1.  In the **Solution Agent**, select the **Edit** button.
2.  In the **Copilot** field, under the Technology heading, enter the following: *“Create an item checkout agent to answer employee questions.”*
3.  Select the **Go** button.
4.  An **Item Checkout Agent** will be added. Select **Keep**.
5.  If you are happy with the suggestions from the **Solution Agent**, select **Looks good**.
6.  Select **Save Tables** build the data model that is suggested.
...
1.  On the **Where do you want to save thigs** screen, set the Name of the solution to Equipment Checkout System and select the **Save** button.
...
1.  Select the **Welcome screen**. *(Provides quick access to items such reservations, checkouts, and extensions.)*
2.  Select the **Reserve entities / Checkouts** screen. *(Allows you to create new, view, edit, and delete reservations / checkouts.)*
3.  Navigate to the additional screens such as **Extensions Requests** and **Notifications** *(Or whichever screens we created for your app)*.
4.  Select the **Welcome screen.**
5.  Select the image above **Reserve entities / checkouts.**
6.  From the menu that appears, select **Edit** \> **Stock** Images.
7.  In the **Search** box enter **Equipment**.
8.  Select one of the suggested images from the list and select the **Insert** button. *(***OPTIONAL:** *Repeat as desired for the remaining images)*
9.  Select the **description** below the Reserve entities / checkouts text.
10. On the **Command bar**, select the **Properties** button. *(Located just to the right of the Editing button.)*
11. Change the **Text** property to: “Start a new or view an existing reservation.”
12. Select the **Reserve entities / Checkouts** text,
13. Change the Text property to “Reserve Equipment.”
14. Select the **Welcome screen** text at the top of the screen.
15. Change the **Text** property to “Contoso Equipment Checkout.”
16. In the **Properties** pane, under **Style and theme**, change the fill color to **Dark Blue**. Select the image above **Staff**.
...
1.  On the command bar, select the **Play** icon. (*Notice the layout of the app will change based on your screen size*)
2.  Select the **Reserve Equipment** image.
3.  To edit the record, select the **Edit** button.
4.  Change the **Name** of the record.
5.  Select the **Check mark** icon to save your changes.
6.  Select the **Home** button to return to **Welcome Screen**.
7.  In the upper right corner of the app, select the **Purple X** to return to the designer.
8.  On the **Command bar**, select the **Save** icon to save your app.
9.  Close your **Browser** tab to return to **Design Planner**. (*Notice the button under your app has changed from create to edit.*)
...
2.  Notice the trigger has automatically been configured to an **Interval** of **1** and a **Frequency** of Day. *(You could specify a specific time of day it should run as well.)*

    ![Screenshot of the recurrence trigger](media/c2b4b1afa9756afd934fbe9e7a740a89.png)

3.  Select the **List rows** step.
...
1.  If there is a value in the **Table Name** field, remove it.
2.  In the **Table name** field, type **Notifications**, and select **Notifications [cX\#X\#_notifications]** or similarly named item in your environment.

    ![Screenshot of the list rows action](media/5b246838ef6bbed257034637d80faccf.png)

3.  Select the **For Each** step.
4.  For each record identified in the **List Rows** step, the flow will send an email to the email address of the employee who has the item checked out to remind them that their item is due back today.
...
1.  On the **Command bar**, select the **Save Draft** button.
2.  Select the **Publish** button.
3.  Close the **Browser** tab to return to **Plan designer** tab.
4.  Notice the **Reminder Notification Flow** now has an **Edit** icon next to it.
